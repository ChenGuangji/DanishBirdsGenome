#!/usr/bin/perl

=head1 Name

stat_TE.pl  --  stat the TE content by type, subtype, and family

=head1 Description

This program can read repeatmasker .out file, RepeatProteinMask .annot file,
and gff file, to statistic the TE content for each type, supertype, subtype, and family.
Note that all the numbers are non-redundant length. 

(1)It can read .annot, .out, and .gff in total 3 formats.
(2)Large type(6): DNA,LTR,LINE,SINE,Other,Unkown;
(3)RC/Helitron own to DNA/Helitron;
(4)No Subtype, subtype named type
(5)Added superfamily: delete - and (), excepted some protected type(DNA/En-Spm,LINE/Dong-R4,LINE/Rex-Babar,Unknown/Y-chromosome,Satellite)
(6)DNA_virus own to Other, eg. Other/DNA_virus

=head1 Usage
  
  perl stat_TE.pl [options]
  --repeat <file>   set repeatmasker result file
  --protein <file>  set RepeatProteinMask result file 
  --gff <file>      set the gff format input file
  --rank <str>      set the rank of stat: all, type, supertype, subtype ,family. default= type
  --fasta <file>    set the genomic fasta
  --verbose   output running progress information to screen  
  --help      output help information to screen  

=head1 Exmple

   perl ./stat_TE.pl --gff ./DBG.repeat.all.gff --fasta ./DBG.genomic.fasta.gz

=cut


use strict;
use Getopt::Long;
use FindBin qw($Bin $Script);
use File::Basename qw(basename dirname); 
use Data::Dumper;
use File::Path;  ## function " mkpath" and "rmtree" deal with directory

##get options from command line into variables and set default values
my (@RepeatMasker,@RepeatProteinMask,@Gff,$Rank,$Fasta);
my ($Verbose,$Help);
GetOptions(
	"repeat:s"=>\@RepeatMasker,
	"protein:s"=>\@RepeatProteinMask,
	"gff:s"=>\@Gff,
	"fasta:s"=>\$Fasta,
	"rank:s"=>\$Rank,
	"verbose"=>\$Verbose,
	"help"=>\$Help
);
$Rank ||= "type";
die `pod2text $0` if ( $Help);
die `pod2text $0` if (! @RepeatMasker && ! @RepeatProteinMask && ! @Gff && ! $Fasta); ##defined(@array) had been discarded after perl v5.22.1 

my (%Data,$Len,$ID);

if($Fasta =~ /gz$/){
	$Len = `zcat $Fasta | perl $Bin/lib/fastaDeal.pl -attribute id:len:lenwogap:gc - | awk '{(tot+=\$2)};END{print tot}'`; chomp ($Len);
	$ID = `basename $Fasta .genomic.fasta.gz`; chomp($ID);
}else{
	$Len = `perl $Bin/lib/fastaDeal.pl -attribute id:len:lenwogap:gc $Fasta | awk '{(tot+=\$2)};END{print tot}'`; chomp ($Len);
	$ID = `basename $Fasta`; chomp($ID);
}
foreach my $file (@RepeatMasker) {
	Read_RepeatMasker($file,\%Data);
}

foreach my $file (@RepeatProteinMask) {
	Read_RepeatProteinMask($file,\%Data);
}

foreach my $file (@Gff) {
	Read_gff($file,\%Data);
}


#foreach my $TE_type (sort keys %Data) {
my (@OUTINFO1p, @OUTINFO2p,@OUTINFO1s, @OUTINFO2s);
foreach my $TE_type (qw(DNA LINE SINE LTR Other Unknown Total)) {
	my $TE_type_p = $Data{$TE_type};
	my $TE_type_size = 0;
	foreach my $chr (sort keys %$TE_type_p) {
		$TE_type_size += Conjoin_fragment($TE_type_p->{$chr});
	}
	push @OUTINFO1p, "$TE_type(%)";
	push @OUTINFO1s, "$TE_type(size)";
 	my $ratio=$TE_type_size/$Len * 100;
	push @OUTINFO2p, "$ratio";
	push @OUTINFO2s, "$TE_type_size";
}

print join("\t","ID",@OUTINFO1p,@OUTINFO1s)."\n";
print join("\t",$ID,@OUTINFO2p,@OUTINFO2s)."\n";

####################################################
################### Sub Routines ###################
####################################################


##read RepeatMasker .out file
#usage: Read_RepeatMasker($file,\%hash);
############################################
sub Read_gff{
	my $file=shift;
	my $hash_p=shift; 
	
	open (IN,$file) || die ("fail open $file\n");
	while (<IN>) {
		chomp;
		s/^\s+//;
		next if(/^\#/);
		my @temp=split(/\t/);

		my $tname = $temp[0];
		my $strand = $temp[6];
		my $start = $temp[3];
		my $end = $temp[4];
		my $TE_name = $1 if ($temp[8] =~ /Target=([^; ]+);*/);
		my $TE_class = $1 if ($temp[8] =~ /Class=([^;]+);*/);
		$TE_class =~ s/\?$//;
		next if($TE_class eq "scRNA" || $TE_class eq "srpRNA" || $TE_class eq "Satellite");
		$TE_class = "Other/DNA_virus" if($TE_class eq "DNA_virus");
		
		
		my ($TE_type,$TE_subtype);
		$TE_type = $1 if($TE_class =~ /^([^\/]+)\/*/);
		$TE_type =~ s/\?$//;
		$TE_type = "DNA" if($TE_type eq "RC");
		$TE_subtype = $1 if($TE_class =~ /^[^\/]+\/([^\/]+)/);
		$TE_subtype ||= $TE_type;
		
		my $TE_supertype = $TE_subtype; ##保护以下几类
		if($TE_subtype ne "En-Spm" && $TE_subtype ne "Dong-R4" && $TE_subtype ne "Rex-Babar" && $TE_subtype ne "Y-chromosome"){
			$TE_supertype =~ s/-[^-]+$//; ##删除-后面子类部分
			$TE_supertype =~ s/\(.+\)$//; ##删除带有括号的子类部分
		}
			
		my $need_stat;
		if ($Rank eq "all") {
			$need_stat = "Total_TE";
		}elsif($Rank eq "type"){
			$need_stat = $TE_type;
		}elsif($Rank eq "supertype"){
			$need_stat = "$TE_type/$TE_supertype";
		}elsif($Rank eq "subtype"){
			$need_stat = "$TE_type/$TE_subtype";
		}elsif($Rank eq "family"){
			$need_stat = "$TE_type/$TE_subtype/$TE_name";
		}
		if ($TE_type eq "DNA" || $TE_type eq "LINE" || $TE_type eq "SINE" || $TE_type eq "LTR" || $TE_type eq "Other" || $TE_type eq "Unknown"){
			push @{$hash_p->{"Total"}{$tname}}, [$start,$end];
		}
		push @{$hash_p->{$need_stat}{$tname}}, [$start,$end]; 
	}
	close(IN);

}


##read RepeatMasker .out file
#usage: Read_RepeatMasker($file,\%hash);
############################################
sub Read_RepeatMasker{
	my $file=shift;
	my $hash_p=shift; 
	
	open (IN,$file) || die ("fail open $file\n");
	while (<IN>) {
		s/^\s+//;
		my @temp=split(/\s+/);
		
		next if($temp[8] ne 'C' && $temp[8] ne '+');
		my $tname = $temp[4];
		my $strand = ($temp[8] eq '+') ? '+' : '-';
		my $start = $temp[5];
		my $end = $temp[6];
		my $TE_name = $temp[9];
		my $TE_class = $temp[10];
		next if($TE_class eq "scRNA" || $TE_class eq "srpRNA" || $TE_class eq "Satellite");
		$TE_class =~ s/\?$//;
		$TE_class = "Other/DNA_virus" if($TE_class eq "DNA_virus");
		
		my ($TE_type,$TE_subtype);
		$TE_type = $1 if($TE_class =~ /^([^\/]+)\/*/);
		$TE_type =~ s/\?$//;
		$TE_type = "DNA" if($TE_type eq "RC");
		$TE_subtype = $1 if($TE_class =~ /^[^\/]+\/([^\/]+)/);
		$TE_subtype ||= $TE_type;

		my $TE_supertype = $TE_subtype; ##保护以下几类
		if($TE_subtype ne "En-Spm" && $TE_subtype ne "Dong-R4" && $TE_subtype ne "Rex-Babar" && $TE_subtype ne "Y-chromosome"){
			$TE_supertype =~ s/-[^-]+$//; ##删除-后面子类部分
			$TE_supertype =~ s/\(.+\)$//; ##删除带有括号的子类部分
		}

		my $need_stat;
		if ($Rank eq "all") {
			$need_stat = "Total_TE";
		}elsif($Rank eq "type"){
			$need_stat = $TE_type;
		}elsif($Rank eq "supertype"){
			$need_stat = "$TE_type/$TE_supertype";
		}elsif($Rank eq "subtype"){
			$need_stat = "$TE_type/$TE_subtype";
		}elsif($Rank eq "family"){
			$need_stat = "$TE_type/$TE_subtype/$TE_name";
		}

		push @{$hash_p->{$need_stat}{$tname}}, [$start,$end]; 
	}
	close(IN);

}



sub Read_RepeatProteinMask{
	my $file=shift;
	my $hash_p=shift; 
	
	open (IN,$file) || die ("fail open $file\n");
	while (<IN>) {
		s/^\s+//;
		next if(/^pValue/);
		my @temp=split(/\s+/);
		my $tname = $temp[3];
		my $strand = $temp[6];
		next if($strand ne '-' && $strand ne '+');
		my $start = $temp[4];
		my $end = $temp[5];
		my $TE_name = $temp[7];
		my $TE_class = $temp[8];
		next if($TE_class eq "scRNA" || $TE_class eq "srpRNA" || $TE_class eq "Satellite");
		$TE_class =~ s/\?$//;
		$TE_class = "Other/DNA_virus" if($TE_class eq "DNA_virus");
		
		my ($TE_type,$TE_subtype);
		$TE_type = $1 if($TE_class =~ /^([^\/]+)\/*/);
		$TE_type =~ s/\?$//;
		$TE_type = "DNA" if($TE_type eq "RC");
		$TE_subtype = $1 if($TE_class =~ /^[^\/]+\/([^\/]+)/);
		$TE_subtype ||= $TE_type;

		my $TE_supertype = $TE_subtype; ##保护以下几类
		if($TE_subtype ne "En-Spm" && $TE_subtype ne "Dong-R4" && $TE_subtype ne "Rex-Babar" && $TE_subtype ne "Y-chromosome"){
			$TE_supertype =~ s/-[^-]+$//; ##删除-后面子类部分
			$TE_supertype =~ s/\(.+\)$//; ##删除带有括号的子类部分
		}		

		my $need_stat;
		if ($Rank eq "all") {
			$need_stat = "Total_TE";
		}elsif($Rank eq "type"){
			$need_stat = $TE_type;
		}elsif($Rank eq "supertype"){
			$need_stat = "$TE_type/$TE_supertype";
		}elsif($Rank eq "subtype"){
			$need_stat = "$TE_type/$TE_subtype";
		}elsif($Rank eq "family"){
			$need_stat = "$TE_type/$TE_subtype/$TE_name";
		}

		push @{$hash_p->{$need_stat}{$tname}}, [$start,$end]; 
	}
	close(IN);

}


##conjoin the overlapped fragments, and caculate the redundant size
##usage: conjoin_fragment(\@pos);
##		 my ($all_size,$pure_size,$redunt_size) = conjoin_fragment(\@pos);
##Alert: changing the pointer's value can cause serious confusion.
sub Conjoin_fragment{
	my $pos_p = shift; ##point to the two dimension input array
	my $distance = shift || 0;
	my $new_p = [];         ##point to the two demension result array
	
	my ($all_size, $pure_size, $redunt_size) = (0,0,0); 
	
	return (0,0,0) unless(@$pos_p);

	foreach my $p (@$pos_p) {
			($p->[0],$p->[1]) = ($p->[0] <= $p->[1]) ? ($p->[0],$p->[1]) : ($p->[1],$p->[0]);
			$all_size += abs($p->[0] - $p->[1]) + 1;
	}
	
	@$pos_p = sort {$a->[0] <=>$b->[0]} @$pos_p;
	push @$new_p, (shift @$pos_p);
	
	foreach my $p (@$pos_p) {
			if ( ($p->[0] - $new_p->[-1][1]) <= $distance ) { # conjoin two neigbor fragements when their distance lower than 10bp
					if ($new_p->[-1][1] < $p->[1]) {
							$new_p->[-1][1] = $p->[1]; 
					}
					
			}else{  ## not conjoin
					push @$new_p, $p;
			}
	}
	@$pos_p = @$new_p;

	foreach my $p (@$pos_p) {
			$pure_size += abs($p->[0] - $p->[1]) + 1;
	}
	
	$redunt_size = $all_size - $pure_size;
	return $pure_size;
}
